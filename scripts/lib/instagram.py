"""Instagram Reels search for /last30days.

Primary: ScrapeCreators API (SCRAPECREATORS_API_KEY).
Fallback: Apify apify/instagram-scraper (APIFY_API_TOKEN).

Both keys are optional; the skill uses whichever is available and warns
on stderr when falling back to Apify.
"""
...
def _parse_apify_items(raw_items: List[Dict[str, Any]], core_topic: str) -> List[Dict[str, Any]]:
    """Parse Apify apify/instagram-scraper items into the same schema as _parse_items()."""
    items = []
    for raw in raw_items:
        if not isinstance(raw, dict):
            continue
        reel_pk = str(raw.get("id") or raw.get("shortCode") or "")
        shortcode = raw.get("shortCode") or raw.get("shortcode") or ""

        caption_obj = raw.get("caption") or raw.get("text") or ""
        if isinstance(caption_obj, dict):
            text = caption_obj.get("text", "")
        else:
            text = str(caption_obj)

        play_count = int(raw.get("videoViewCount") or raw.get("videoPlayCount") or raw.get("plays") or 0)
        like_count = int(raw.get("likesCount") or raw.get("likes") or 0)
        comment_count = int(raw.get("commentsCount") or raw.get("comments") or 0)

        owner = raw.get("ownerUsername") or raw.get("username") or ""

        date_str = None
        ts = raw.get("timestamp") or raw.get("taken_at")
        ...
        hashtags = [h if isinstance(h, str) else h.get("name", "") for h in raw_hashtags if h]
...
def _search_instagram_apify(
    ...
    """Search Instagram via Apify apify/instagram-scraper (fallback path)."""
    from . import apify as _apify
    config = DEPTH_CONFIG.get(depth, DEPTH_CONFIG["default"])
    core_topic = _extract_core_subject(topic)

    sys.stderr.write("[last30days] WARNING: Instagram ScrapeCreators unavailable; using Apify fallback\n")
    sys.stderr.flush()

    raw_items = _apify.instagram_search(core_topic, count=config["results_per_page"], token=apify_token)
    if not raw_items:
        _log("Apify Instagram fallback returned no results")
        return {"items": [], "error": "Apify Instagram returned no results"}

    items = _parse_apify_items(raw_items, core_topic)

    in_range = [i for i in items if i["date"] and from_date <= i["date"] <= to_date]
    if in_range:
        items = in_range
    items.sort(key=lambda x: x["engagement"]["views"], reverse=True)
    _log(f"Apify fallback: found {len(items)} Instagram reels")
    return {"items": items}
...
def search_instagram(
    ...
    """Search Instagram Reels via ScrapeCreators API with Apify fallback.
    ...
    if not token:
        return _search_instagram_apify(topic, from_date, to_date, depth, apify_token)
...
def search_and_enrich(
    ...
    runs ScrapeCreators for each (falling back to Apify if SC fails),
    and merges/deduplicates results by video ID.
    ...
    for q in queries:
        search_result = search_instagram(q, from_date, to_date, depth, token, apify_token=apify_token)
