...
def is_tiktok_available(config: dict[str, Any]) -> bool:
    """Check if TikTok source is available (ScrapeCreators or Apify fallback).
...
def get_tiktok_token(config: dict[str, Any]) -> str:
    """Get ScrapeCreators token for TikTok. Returns empty string if not configured."""
    return config.get('SCRAPECREATORS_API_KEY') or ''


def get_apify_token(config: dict[str, Any]) -> str:
    """Get Apify API token for fallback scraping (TikTok, Instagram, Threads)."""
...
def is_threads_available(config: dict[str, Any]) -> bool:
    """Check if Threads source is available (ScrapeCreators or Apify fallback).

    Requires 'threads' in INCLUDE_SOURCES plus either SC or Apify key.
    Threads is an opt-in source - it is not activated by default.
    """
    has_key = bool(config.get('SCRAPECREATORS_API_KEY') or config.get('APIFY_API_TOKEN'))
    if not has_key:
        return False
...
def is_instagram_available(config: dict[str, Any]) -> bool:
    """Check if Instagram source is available (ScrapeCreators or Apify fallback).
...
def get_instagram_token(config: dict[str, Any]) -> str:
    """Get ScrapeCreators token for Instagram. Returns empty string if not configured."""
    return config.get('SCRAPECREATORS_API_KEY') or ''
...
def is_scrapling_available(config: dict[str, Any] | None = None) -> bool:
    """Check if Scrapling package is installed and importable.

    No API key required — availability depends solely on whether the
    'scrapling' Python package is installed (pip install scrapling).
    """
    try:
...
def is_duckduckgo_available(config: dict[str, Any] | None = None) -> bool:
    """Check if duckduckgo-search package is installed.

    No API key required. Install: pip install duckduckgo-search.
    """
    try:
...
def is_gdelt_available(config: dict[str, Any] | None = None) -> bool:
    """GDELT is always available — free public REST API, no auth or package needed."""
    return True


def is_google_news_available(config: dict[str, Any] | None = None) -> bool:
    """Google News RSS is always available — stdlib urllib only, no auth needed."""
    return True


def is_mastodon_available(config: dict[str, Any] | None = None) -> bool:
    """Mastodon is always available via mastodon.social public API.

    Set MASTODON_INSTANCE to use a different instance.
    """
    return True


def get_mastodon_instance(config: dict[str, Any]) -> str:
    """Return configured Mastodon instance hostname (default: mastodon.social)."""
    return (config.get("MASTODON_INSTANCE") or "mastodon.social").strip()


def is_devto_available(config: dict[str, Any] | None = None) -> bool:
    """Dev.to API is always available — free, no key, pure HTTP."""
    return True


def is_stackexchange_available(config: dict[str, Any] | None = None) -> bool:
    """Stack Exchange API is always available — no key needed for basic tier."""
    return True


def get_stackexchange_api_key(config: dict[str, Any]) -> str:
    """Return Stack Exchange API key for higher rate limits (optional)."""
    return config.get("STACKEXCHANGE_API_KEY") or ""


def is_arxiv_available(config: dict[str, Any] | None = None) -> bool:
    """arXiv API is always available — stdlib urllib only, no auth needed."""
    return True


def is_lemmy_available(config: dict[str, Any] | None = None) -> bool:
    """Lemmy is always available via public REST API."""
    return True


def get_lemmy_instance(config: dict[str, Any]) -> str:
    """Return configured Lemmy instance hostname (default: lemmy.world)."""
    return (config.get("LEMMY_INSTANCE") or "lemmy.world").strip()


def is_peertube_available(config: dict[str, Any] | None = None) -> bool:
    """PeerTube is always available via public search API."""
    return True


def get_peertube_instance(config: dict[str, Any]) -> str:
    """Return configured PeerTube instance hostname (default: peertube.social)."""
    return (config.get("PEERTUBE_INSTANCE") or "peertube.social").strip()


def get_browseros_mcp_url(config: dict[str, Any] | None = None) -> str:
    """Return BrowserOS MCP endpoint (default: http://127.0.0.1:9000/mcp)."""
    url = (config or {}).get("BROWSEROS_MCP_URL") or "http://127.0.0.1:9000/mcp"
    return url.strip()


def is_browseros_available(config: dict[str, Any] | None = None) -> bool:
    """Return True if BrowserOS MCP server is reachable on its port."""
    from .browseros_scraper import is_available
    return is_available(get_browseros_mcp_url(config))


def is_exa_mcp_available(config: dict[str, Any]) -> bool:
    """Exa neural search available when EXA_API_KEY is set AND 'exa_mcp' in INCLUDE_SOURCES.

    Kept opt-in to avoid double-spending Exa credits when grounding already
    fires the keyword-Exa backend on the same subqueries.
    """
    if not config.get("EXA_API_KEY"):
        return False
    return "exa_mcp" in _parse_include_sources(config)


def is_browseros_x_available(config: dict[str, Any] | None = None) -> bool:
    """Return True if BrowserOS is running AND has an active X/Twitter session."""
    if not is_browseros_available(config):
        return False
    from .browseros_x import has_x_session
    return has_x_session(get_browseros_mcp_url(config))


def get_extra_rss_feeds(config: dict[str, Any]) -> list[str]:
    """Return list of extra RSS/Atom feed URLs from EXTRA_RSS_FEEDS config key."""
    raw = (config.get("EXTRA_RSS_FEEDS") or "").strip()
    if not raw:
        return []
    return [url.strip() for url in raw.split(",") if url.strip()]


def is_rss_available(config: dict[str, Any]) -> bool:
    """RSS source is available when EXTRA_RSS_FEEDS is configured."""
    return bool(get_extra_rss_feeds(config))
