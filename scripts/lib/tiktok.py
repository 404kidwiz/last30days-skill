...
Fallback: Apify clockworks/tiktok-scraper (APIFY_API_TOKEN).
...
def _parse_apify_items(raw_items: List[Dict[str, Any]], core_topic: str) -> List[Dict[str, Any]]:
    """Parse Apify clockworks/tiktok-scraper items into the same schema as _parse_items()."""
    items = []
    for raw in raw_items:
        if not isinstance(raw, dict):
            continue
        video_id = str(raw.get("id") or raw.get("aweme_id") or "")
        text = raw.get("desc") or raw.get("text") or raw.get("description") or ""
        url = raw.get("webVideoUrl") or raw.get("share_url") or ""

        author_meta = raw.get("authorMeta") or {}
        if isinstance(author_meta, dict):
            author_name = author_meta.get("name") or author_meta.get("uniqueId") or ""
        else:
            author_name = str(author_meta) if author_meta else ""

        date_str = None
        iso = raw.get("createTimeISO")
        ...
        digg_count = int(raw.get("diggCount") or 0)
        ...
        for h in raw_hashtags:
            if isinstance(h, dict):
                hashtag_names.append(h.get("name") or h.get("title") or "")
...
def _search_tiktok_apify(
    ...
    """Search TikTok via Apify clockworks/tiktok-scraper (fallback path)."""
    from . import apify as _apify
    config = DEPTH_CONFIG.get(depth, DEPTH_CONFIG["default"])
    core_topic = _extract_core_subject(topic)

    sys.stderr.write("[last30days] WARNING: TikTok ScrapeCreators unavailable; using Apify fallback\n")
    sys.stderr.flush()

    raw_items = _apify.tiktok_search(core_topic, count=config["results_per_page"], token=apify_token)
    if not raw_items:
        _log("Apify TikTok fallback returned no results")
        return {"items": [], "error": "Apify TikTok returned no results"}
    ...
    _log(f"Apify fallback: found {len(items)} TikTok videos")
    return {"items": items}
...
def search_tiktok(
    ...
    Returns:
        ...
        apify_token: Apify API token for fallback when ScrapeCreators fails or is absent.
    """
    if not token and not apify_token:
        return {"items": [], "error": "No SCRAPECREATORS_API_KEY or APIFY_API_TOKEN configured"}

    if not token:
        return _search_tiktok_apify(topic, from_date, to_date, depth, apify_token)
...
def search_and_enrich(
    ...
    for q in queries:
        search_result = search_tiktok(q, from_date, to_date, depth, token, apify_token=apify_token)
