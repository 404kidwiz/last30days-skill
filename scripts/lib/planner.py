...
QUICK_SOURCE_PRIORITY = {
    "factual": ["hackernews", "reddit", "x", "youtube", "stackexchange"],
    "product": ["youtube", "reddit", "x", "tiktok", "devto"],
    "concept": ["hackernews", "reddit", "x", "youtube", "arxiv"],
    "opinion": ["reddit", "x", "youtube", "hackernews", "mastodon"],
    "how_to": ["youtube", "reddit", "x", "hackernews", "stackexchange"],
    "comparison": ["reddit", "x", "hackernews", "youtube", "stackexchange"],
    "breaking_news": ["x", "reddit", "hackernews", "gdelt", "google_news", "youtube", "polymarket"],
    "prediction": ["polymarket", "x", "hackernews", "reddit", "youtube"],
}
SOURCE_PRIORITY = {
    "factual": ["hackernews", "reddit", "x", "youtube", "stackexchange", "arxiv"],
    "product": ["youtube", "reddit", "x", "tiktok", "hackernews", "devto"],
    "concept": ["hackernews", "reddit", "x", "youtube", "arxiv", "devto"],
    "opinion": ["reddit", "x", "youtube", "hackernews", "mastodon", "lemmy"],
    "how_to": ["youtube", "reddit", "x", "hackernews", "stackexchange", "devto"],
    "comparison": ["reddit", "x", "hackernews", "youtube", "stackexchange"],
    "breaking_news": ["x", "reddit", "hackernews", "gdelt", "google_news", "youtube", "polymarket"],
    "prediction": ["polymarket", "x", "hackernews", "reddit", "youtube"],
}
...
INTENT_SOURCE_EXCLUSIONS: dict[str, set[str]] = {
    # GDELT is a news event DB — low signal for non-news intents
    # Mastodon is social noise for factual/tutorial/product searches
    "concept": {"polymarket", "gdelt"},
    "how_to": {"polymarket", "gdelt", "mastodon"},
    "factual": {"gdelt", "mastodon"},
    "product": {"gdelt", "mastodon"},
    "breaking_news": {"arxiv", "peertube"},
    "prediction": {"arxiv", "peertube", "devto", "gdelt"},
}
SOURCE_CAPABILITIES = {
    ...
    "gdelt": {"web", "reference", "link", "news"},
    "google_news": {"web", "reference", "link", "news"},
    "mastodon": {"discussion", "social"},
    "duckduckgo": {"web", "reference", "link"},
    "scrapling": {"web", "reference", "link"},
    "devto": {"discussion", "link"},
    "stackexchange": {"discussion", "link"},
    "arxiv": {"reference", "link", "academic"},
    "lemmy": {"discussion", "social"},
    "peertube": {"video", "video_longform"},
    "rss": {"web", "reference", "link", "news"},
    "browseros": {"web", "reference", "link"},
    "browseros_x": {"discussion", "social"},
    "exa_mcp": {"web", "reference", "link"},
}
DEFAULT_INTENT_CAPABILITIES = {
    "comparison": {"discussion", "video", "web", "reference", "social", "link", "market"},
    "how_to": {"discussion", "video", "web", "reference", "link", "academic"},
    "breaking_news": {"web", "reference", "link", "news", "discussion", "social"},
}
SOURCE_DESCRIPTIONS = {
    "reddit": "Reddit — community discussions, threads, upvotes",
    "x": "X (Twitter) — real-time posts, trending topics",
    "youtube": "YouTube — long-form video, tutorials, vlogs",
    "tiktok": "TikTok — short-form video, viral content",
    "instagram": "Instagram — Reels, short-form video, lifestyle",
    "hackernews": "Hacker News — tech discussion, link aggregator, Show HN",
    "bluesky": "Bluesky — decentralized social posts",
    "truthsocial": "Truth Social — conservative social media posts",
    "polymarket": "Polymarket — prediction market odds and forecasts",
    "digg": "Digg — link aggregation, community upvotes",
    "xiaohongshu": "Xiaohongshu (Rednote) — Chinese lifestyle short video",
    "github": "GitHub — code, issues, PRs, developer discussions",
    "grounding": "Web grounding — broad web search (Brave, Exa, Serper)",
    "perplexity": "Perplexity — AI-synthesized web analysis",
    "gdelt": "GDELT — global news event database, 100+ languages, always free",
    "google_news": "Google News RSS — curated news headlines, always free",
    "mastodon": "Mastodon — federated open social posts (mastodon.social)",
    "duckduckgo": "DuckDuckGo — privacy-first web/news search, always free",
    "scrapling": "Scrapling — DuckDuckGo HTML scrape, always free (no dates)",
    "devto": "Dev.to — developer articles, tutorials, community posts",
    "stackexchange": "Stack Exchange — Q&A: Stack Overflow + network, always free",
    "arxiv": "arXiv — academic preprints: CS, ML, physics, math, always free",
    "lemmy": "Lemmy — federated Reddit-like communities, always free",
    "peertube": "PeerTube — federated open-source video platform, always free",
    "rss": "RSS — custom feeds from EXTRA_RSS_FEEDS env var",
    "browseros": "BrowserOS — real browser DDG search, bypasses anti-bot",
    "browseros_x": "BrowserOS X — real browser X/Twitter search using active session",
    "exa_mcp": "Exa Neural — semantic/conceptual web search (type=neural), finds conceptually related content",
    "threads": "Threads — Meta's text social network",
    "pinterest": "Pinterest — visual discovery, image boards",
    "xquik": "X Quick — fast X/Twitter snippet extraction",
}
...
def _build_prompt(
    ...
    source_lines = "\n".join(
        f"  - {src}: {SOURCE_DESCRIPTIONS.get(src, src)}"
